...
const ProjectCreateDialogComponent = (props) => {
    ...
    /**
     * Before this the array date objects become string when called from service
     * now it been transformed into date when called from service
     */
    // const _project_duration = _entity?.project_duration ? _entity?.project_duration.map((el) => (el ? new Date(el) : null)) : [];
...
export default connect(null, mapDispatch)(ProjectCreateDialogComponent);
